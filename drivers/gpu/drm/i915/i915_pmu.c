 ...
 * minimum that can avoid the default kernel.perf_event_max_sample_rate
					     ...
					     struct perf_event *event)
{
	 ...
	 * byte aligned. (See note in alloc_oa_buffer for more details) */
	raw.size = snapshot_size + 4;
	raw.data = snapshot;

	data.raw = &raw;

	/* XXX: Due to the bursty nature of forwarding OA reports to perf, perf
	 * may think we're exceeding the perf sampling threshold which may not
	 * be true considered over a longer period of time. */
#warning "XXX: how to avoid perf complaining that we're exceeding the sampling threshold while flushing"
				    ...
				    bool skip_if_flushing)
{
	...
	/* TODO: write records to the perf circular buffer to convey
	 * this OA unit status info to userspace. Perf doesn't
	 * currently allow us to define custom records. */
	if (unlikely(oastatus1 & (GEN7_OASTATUS1_OABUFFER_OVERFLOW |
				  GEN7_OASTATUS1_REPORT_LOST))) {

		if (oastatus1 & GEN7_OASTATUS1_OABUFFER_OVERFLOW)
			DRM_DEBUG_DRIVER("OABUFFER overflow");

		if (oastatus1 & GEN7_OASTATUS1_REPORT_LOST)
			DRM_DEBUG_DRIVER("OA report lost");
...
static void i915_oa_event_destroy(struct perf_event *event)
{
	...
	if (dev_priv->oa_pmu.specific_ctx) {
		...
		i915_gem_context_unreference(dev_priv->oa_pmu.specific_ctx);

		/* Stop updating oacontrol via _context_pin_[un]notify()... */
...
static int i915_oa_event_init(struct perf_event *event)
{
	...
	if (dev_priv->oa_pmu.periodic) {
		...
		if (period_exponent > OA_EXPONENT_MAX) {
			DRM_ERROR("Sampling exponent too high\n");
			return -EINVAL;
		}
...
void i915_oa_pmu_register(struct drm_device *dev)
{
	...
	hrtimer_init(&dev_priv->oa_pmu.timer,
